...
function setup() {
  noCanvas();
  if (window.location.href.search(/\?=search/) >0){searchable=true;}
  frm=document.getElementById('folders');
  stashurl=frm.src;
  if (searchable){
  	stashurl='../php/jukebox.php';
  }
  frm.src=stashurl;
...
function draw() {
	...
	let allow=searchable;
	if(!searchable){if (frm.contentWindow.location!=stashurl){allow=true}}
	if (allow){
	let title=frm.contentWindow.document.getElementsByTagName('h1')[0];
		...
		if(ul){ul.style.height=windowHeight*0.78+'px'; ul.style.overflow="auto";}
		...
		if (searchable){
			frm.contentWindow.document.getElementsByTagName('audio')[0].onplay=function(){frm.contentWindow.document.getElementsByTagName('audio')[0].pause();};
			frm.contentWindow.document.getElementsByTagName('audio')[0].
			frm.contentWindow.document.getElementsByTagName('audio')[0].style.visibility='hidden'
		}
	}
...
function del(){
	...
	for (let i=0; i<dropbox.options.length;i++){
		let pp=dropbox[i];
		pp.ondblclick=function(){playSong(i);pp.scrollIntoView();}
		pp.style.userSelect="all";
	}
...
function highlight(currSong){
	...
	dropbox.options[currSong].scrollIntoView();
}
