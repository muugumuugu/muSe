...
weighted1= (df["signal_mean"]+0.5*df["signal_std"] + df["signal_skew"] + df["signal_kurtosis"] +df["tempow"]) * (df["mfccs_mean"]+0.5*df["mfccs_std"]) * (df["chroma_stft_mean"]+0.5*df["chroma_stft_std"])*(10**-4)
